...
class TestElectronBuilderConfig:
    ...
    def test_config_windows_portable_enabled(self):
        ...
        assert "target: 'portable'" in content or \
               "target: \"portable\"" in content
...
class TestPythonBinaryEmbedding:
    ...
    def test_embed_script_has_embedPythonBinary_function(self):
        ...
        assert 'embedPythonBinary' in content, \
            'Script should have embedPythonBinary function'
        assert 'function embedPythonBinary' in content or \
               'async function embedPythonBinary' in content
    ...
    def test_embed_script_handles_linux(self):
        ...
        script_path = Path(__file__).parent.parent / 'build' / 'embed_python_binary.js'
        content = script_path.read_text()
        assert 'linux' in content or 'os.platform' in content, \
            'Script should handle Linux'
    ...
    def test_embed_script_verifies_file_size(self):
        ...
        assert '1000000' in content or '1MB' in content, \
            'Script should verify file size'
...
class TestMainJSIntegration:
    ...
    def test_main_js_handles_extraction_error(self):
        ...
        assert 'try' in content and 'catch' in content, \
            'main.js should have error handling'
...
class TestPackageJsonBuildScripts:
    ...
    def test_package_json_has_build_python_script(self):
        ...
        assert 'build-python' in package_json['scripts'], \
            'Should have build-python script'
    ...
    def test_package_json_has_embed_python_script(self):
        ...
        with open(package_path, encoding='utf-8') as f:
            package_json = json.load(f)
        assert 'embed-python' in package_json['scripts'], \
            'Should have embed-python script'
...
class TestBuildProcess:
    ...
    def test_prebuild_portable_script_exists(self):
        ...
        with open(package_path, encoding='utf-8') as f:
            package_json = json.load(f)
        scripts = package_json.get('scripts', {})
        assert 'prebuild-portable' in scripts, \
            'prebuild-portable script should exist'
