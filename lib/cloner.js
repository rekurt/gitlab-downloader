...
export async function ensureCredentialsInHelper(repoUrl, token, options = {}) {
  ...
  try {
    ...
    throw new Error('Credential helper mode supports only http(s) repository urls');
  }
  ...
  // Serialize credential operations per host by chaining promises
  const existingLock = _credentialLocks.get(host) || Promise.resolve();
  const work = existingLock.then(async () => {
    if (_credentialReadyHosts.has(host)) {
      return;
    }

    const protocol = parsed.protocol.replace(':', '');
    const approvePayload = `protocol=${protocol}\nhost=${host}\nusername=oauth2\npassword=${token}\n\n`;
...
export async function cloneRepository(project, config, options = {}) {
  ...
  if (exists) {
    ...
    if (config.gitAuthMode === 'credential_helper') {
      ...
      pullArgs = ['git', '-C', targetPath, 'pull', '--ff-only'];
    } else {
      // For url auth mode, pass the authenticated URL directly to git pull
      // so credentials are available even if the remote was cloned with a token URL
      const authUrl = buildAuthenticatedCloneUrl(httpsUrl, config.token || '');
      pullArgs = ['git', '-C', targetPath, 'pull', '--ff-only', authUrl];
    }

    logger('info', 'Updating %s with git pull --ff-only', repoName);
    const { code, stderr } = await runGit(pullArgs, {
      signal,
    });
