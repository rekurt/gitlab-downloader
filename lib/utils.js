...
export function isSubpath(basePath, targetPath) {
  try {
    const baseReal = realpathSync(resolve(basePath));
    const targetReal = realpathSync(resolve(targetPath));
    return targetReal.startsWith(baseReal + sep) || targetReal === baseReal;
  } catch {
    return false;
  }
}
