...
describe('fetchJson', () => {
  ...
  it('sends PRIVATE-TOKEN header when token is provided', async () => {
    ...
    expect(calledHeaders['PRIVATE-TOKEN']).toBe('my-secret-token');
  });

  it('does not send PRIVATE-TOKEN when token is null', async () => {
    const mockFetch = jest.fn().mockResolvedValue(mockResponse({}));
    ...
    const calledHeaders = mockFetch.mock.calls[0][1].headers;
    expect(calledHeaders['PRIVATE-TOKEN']).toBeUndefined();
  });
...
describe('fetchPaginated', () => {
  ...
  it('stops on null response (error)', async () => {
  ...
  it('stops on non-array response', async () => {
