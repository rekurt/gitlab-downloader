...
describe('ensureCredentialsInHelper', () => {
  ...
  it('allows http URLs', async () => {
    ...
    await ensureCredentialsInHelper('http://gitlab.com/repo.git', 'token', { runGit });
    expect(runGit.calls[0].options.stdinText).toContain('protocol=http');
    expect(runGit.calls[0].options.stdinText).toContain('host=gitlab.com');
  });

  it('throws for non-http(s) URLs', async () => {
    await expect(
      ensureCredentialsInHelper('ftp://gitlab.com/repo.git', 'token')
    ).rejects.toThrow('supports only http(s)');
  });
